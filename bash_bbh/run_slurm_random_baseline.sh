#!/usr/bin/env bash
#SBATCH --job-name=smart-random-baseline-bbh
#SBATCH --nodelist=bob
...
echo "=== Running Random Baseline for BBH Dataset ==="

# 基础参数配置（BBH数据集专用）
BASE_EXTRA=(
  --approach=beam_search
  --smart_search=True
  --score_method=random_score
  --n=1
  --beam_width=16
  --num_iterations=40
  --temperature=0.8
...
echo "=== Running: python scripts/test_time_compute.py ${BASE_EXTRA[*]} --random_thresholds ${RANDOM_THRESHOLDS[*]} ==="
...
echo "=== Results saved to: /storage/ukp/work/xie12/uncertainty-guided-reasoning/UQ_Guided_Router/outputs/smart/random_score/smart_random_score/ ==="
