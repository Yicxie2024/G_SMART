...
def _beam_search_random_score(
    batch_of_prompts, config: Config, slm: LLM, prm: PRM, llm: None, random_threshold: float
) -> tuple:
    """Beam search with random score-based correction.
    
    Args:
        random_threshold: Threshold for random score. If random_score > random_threshold, use LLM correction.
    """
    ...
    for iterate_idx in tqdm(
        range(config.num_iterations), desc=f"Random score beam search (threshold={random_threshold:.3f})"
    ):
        ...
        # Generate random scores for each active beam (0-1 range)
        random_scores = []
        for beam in active_beams:
            # Generate a random score between 0 and 1
            random_score = random.random()
            random_scores.append([random_score])
            # Store random score in all_scores for tracking
            beam.all_scores.append(random_score)

        # Filter for incomplete beams for potential correction
        random_scores_filtered = [
            random_scores[i] for i, b in enumerate(active_beams) if not b.completed
        ]
        ...
        # Random score-based correction: if random_score > random_threshold, use LLM to correct
        need_correction = random_scores_filtered and random_scores_filtered[0][0] > random_threshold
        ...
        # Keep the random scores from the original beam
...
def smart_beam_search_random_score(examples, config: Config, slm: LLM, prm: PRM, llm: None):
    """Random score-based SMART beam search for multiple thresholds.
    
    This function runs random score-based correction for each threshold in config.random_thresholds.
    ...
    Returns:
        dict: Dictionary containing results for all thresholds. Keys are formatted as:
              - "random_thresholds": list of threshold values used
    ...
    # Get random thresholds from config
    random_thresholds = getattr(config, 'random_thresholds', [])
    if not random_thresholds:
        raise ValueError("config.random_thresholds must be provided and non-empty")
    
    # Results dictionary: flatten all threshold results into a single dict
    # with threshold-prefixed field names
    # Note: random_thresholds is not stored per-sample to avoid Arrow schema issues
    # It's available in config.random_thresholds and used by split_dataset_by_thresholds
    all_results = {}
    
    # Run beam search for each threshold
    for random_threshold in random_thresholds:
        # Format threshold as string for use in field names
        # Use a format that's filesystem-safe and clear
        threshold_str = f"{random_threshold:.6f}".rstrip('0').rstrip('.')
        if threshold_str == "":
            threshold_str = "0"
        threshold_str = threshold_str.replace('.', '_')  # Replace dots with underscores for field names
        
        # Run beam search with this threshold
        beam_results, total_tokens, prm_scores = _beam_search_random_score(
            problems, config, slm, prm, llm, random_threshold
        )
...
def split_dataset_by_thresholds(dataset, config: Config):
    ...
    This function takes a dataset that was processed with smart_beam_search_random_score
    ...
    Args:
        ...
        config: Config object with random_thresholds attribute
    ...
    for random_threshold in random_thresholds:
        ...
        for i in range(len(dataset)):
            ...
            for key, value in example.items():
                # Skip random_thresholds field
                if key == "random_thresholds":
                    continue
