...
def _beam_search_llm_only_single_pass(
    batch_of_prompts, config: Config, llm: LLM, prm: PRM = None
) -> tuple:
    """LLM-only baseline with single-pass generation: generate everything in one go.
    
    This uses the LLM directly without any SLM or correction mechanism.
    Uses vLLM for efficient generation, similar to SLM-only baseline.
    """
    ...
    outputs = llm.generate(templated_convs, sampling_params, use_tqdm=False)
...
def smart_beam_search_llm_only(examples, config: Config, llm: LLM, prm: PRM = None):
    """LLM-only baseline: no corrections, no SLM, just LLM generation.
    
    This is a standalone function that only runs LLM-only baseline,
    without running UQ-guided method first (unlike smart_beam_search_conf).
    This saves computation resources when you only need LLM-only baseline.
    ...
    - pred
    - scores
    - correction_counts (always 0 for LLM-only)
    - completion_times_uq
    - llm_correction_tokens_uq (always 0 for LLM-only, since all tokens are LLM tokens)
    - smart_step (always [-1] for LLM-only)
    - total_tokens
    - correction_token_ratio (always 0.0 for LLM-only)
    """
    problems = examples["problem"]
    
    # Run LLM-only baseline with single-pass generation
    beam_results_llm, total_tokens_llm, prm_scores_llm = _beam_search_llm_only_single_pass(
    ...
    for p in problems:
        ...
        # LLM-only has no corrections, so correction_counts is always 0
        counts_llm = [0 for _ in beams_llm]
        # LLM-only: all tokens are LLM tokens, but llm_correction_tokens_uq tracks correction tokens
        # Since there are no corrections, this is 0
        tokens_llm = [0 for _ in beams_llm]
        # LLM-only has no smart steps (all are -1)
        smart_steps_llm = [getattr(b, "smart_step", [-1]) for b in beams_llm]
        
        # Calculate total_tokens and correction_token_ratio for each beam
        beam_total_tokens = []
        beam_correction_ratios = []
        for beam in beams_llm:
            # Calculate total tokens for this beam: only LLM tokens
            llm_tokens = sum(getattr(beam, "completion_tokens", []))
            total_tokens_beam = llm_tokens
            beam_total_tokens.append(total_tokens_beam)
            
            # Correction token ratio is always 0.0 for LLM-only (no corrections)
            beam_correction_ratios.append(0.0)
        
        results["completions"].append(completions_llm)
        results["pred"].append(pred_llm)
        results["scores"].append(scores_llm)
        results["correction_counts"].append(counts_llm)
        results["completion_times_uq"].append(times_llm)
        results["llm_correction_tokens_uq"].append(tokens_llm)
        results["smart_step"].append(smart_steps_llm)
